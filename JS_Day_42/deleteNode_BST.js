...
class BinaryTree {
    ...
    // actual implementation of deleting the node in BST
    deleteNode(root, value) {
        // when root will be null, underflow situation arises
        if (root === null) {
            return null;
        }
        // if deleting value will be less than root's value, it recursively calls deleteNode function
        if (value < root.value) {
            root.left = this.deleteNode(root.left, value);
        }
        // if deleting value will be greater than root's value, it recursively calls deleteNode function
        else if (value > root.value) {
            root.right = this.deleteNode(root.right, value);
        }
        // otherwise
        else {
            // if neither the left nor the right value exist, it returns null
            if (!root.left && !root.right) {
                return null;
            }
            // if root's left doesn't exist checks the root'right 
            else if (!root.left) {
                return root.right;
            }
            // if root's right doesn't exist checks the root's left
            else if (!root.right) {
                return root.left;
            }
